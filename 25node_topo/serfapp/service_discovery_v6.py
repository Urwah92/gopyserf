...
Behavior:
- Each cycle:
  1) (Optionally) fetch buyer request from --buyer-url and apply thresholds/budgets/scores.
  2) Load geometry + LAN members, run RTT-first then raw-Hilbert widening.
  3) Publish results to a live HTTP endpoint (--http-serve) that serves the latest payload.
  4) Sleep a fixed --busy-secs, then repeat (if --loop).

The HTTP endpoint stays live and keeps serving the most recent results while sleeping.

Example:
  python3 service_discovery_v6.py \
...
def filter_by_resources(
    ...
    # Per-unit price caps: treat NaN as +inf
    ...
    # Per-unit score floors: treat NaN as 0.0
...
def apply_buyer_overrides(args, ap):
    """Refresh args from buyer URL (demands -> mins; budget/score when >0)."""
...
# ----------------------- One discovery run --------------------
def run_once(query_node: str, args) -> dict:
...
def main():
    ...
    httpd = None
    if args.http_serve:
        httpd = start_live_http_server(state, args.http_host, args.http_port, args.http_path)

    def do_cycle():
        # refresh thresholds from buyer each cycle (if provided)
        if args.buyer_url:
            apply_buyer_overrides(args, ap)
        payload = run_once(query_node, args)
        with state["lock"]:
    ...
    try:
        if args.loop:
            ...
            if not args.http_serve:
                # pretty-print to stdout when not serving
                if payload.get("results"):
                    df = pd.DataFrame(payload["results"]).set_index("name")
                    print(df.replace({np.nan: None}).to_string())
                else:
                    print("(no results)")
            else:
